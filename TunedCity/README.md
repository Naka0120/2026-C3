# TunedCity — Wind Visualization

## プロジェクト概要

ファネル型ビル（2棟）による整流・加速効果をブラウザ上で3D可視化するプロジェクトです。
...
- CFD 風速場は `../openfoam_case/wind_field.json` から読み込む（事前に CFD 計算が必要）
- Multipass VM（openfoam）は CFD を再計算するときのみ必要。確認だけなら不要
- ポート 5050 が使用中の場合は `lsof -ti:5050 | xargs kill -9` で解放する

## CFD 再計算手順（ビル形状を変更した場合）

```bash
# 1. ケース再生成
curl http://127.0.0.1:5050/api/generate-cfd-case

# 2. Multipass VM で計算実行（SSH が繋がる状態で）
multipass exec openfoam -- bash -c \
  "source /opt/openfoam13/etc/bashrc && cd /home/ubuntu/case && \
...
RansCFDWindSolver(Path('../openfoam_case')).extract_wind_field()
"
```
